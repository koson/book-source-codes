/* Segment into isolated characters using break cost */
...
void main (int argc, char *argv[])
{
	...
	if (argc < 2)
	{
	  printf ("Usage: segcost <image>\n");
	  printf ("Segmentation using break cost \n");
	  exit (1);
	}
...
void segment (IMAGE x, int line)
{
	...
	for (j=1; j<x->info->nc; j++)
	{
	  k = 0;
	  for (i=rs; i<re; i++)
	    if (x->data[i][j] > 0 && x->data[i][j-1]>0) k++;
	  vproj[j] = k;
	}
	...
	for (j=1; j<x->info->nc-1; j++)
	{
	  k = 0;
	  for (i=rs; i<re; i++)
	    if ((x->data[i][j] > 0 && x->data[i][j-1]>0) ||
		(x->data[i][j+1] > 0 && x->data[i][j] > 0)) k++;
	  vproj[j] *= k;
	  printf ("%3d  %3d\n", j, vproj[j]);
	}
}
