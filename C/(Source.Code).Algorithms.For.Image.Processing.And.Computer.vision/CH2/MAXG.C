...
int main(int argc, char *argv[])
{
	...
	if (infile == NULL)
	{
		printf ("Can't open %s input file '%s'.\n", argv[0], argv[1]);
		exit(1);
	}
	...
	if (errors == 0)
	{
	  if (COMPILER == TURBO)
	  {
	    sprintf (compile,"tcc  -mh %s grey.c", cname);
	    system (compile);
	    fprintf (stderr, "C compilation complete.\n");
	  } else if (COMPILER == GCC)
	  {
		printf ("Now you must compile the C version using:\n");
		printf ("  gcc -o %s %s grey.c -lm\n", binname, cname);
		printf ("  go32 -s c:\\gnu\\bin go32.exe %s\n", binname);
		printf ("\n\n");
	  }
	}
	return 0;
